                ...
                if (!state.jewels.contains(jewelMoves[i]) && state.emptys.contains(jewelMoves[i]))
                {
                    try
                    {
                        ...
                        if (manMoves[i].equals(state.man))
                        {
                            ...
                            manGoal.manMovesInState = "";
                            //Test to see if last movement was with CAN
                            switch (i)
                            {
                                case 0:
                                	if(!state.manMovesInState.endsWith(Character.toString(upDirection).toUpperCase()))
                                	{
                                		//manGoal.manMovesInState = state.manMovesInState.substring(state.manMovesInState.length() - 1);
                                		manGoal.manMovesInState += 'a';
                                    ...
                                    break;
                                case 1:
                                	if(!state.manMovesInState.endsWith(Character.toString(downDirection).toUpperCase()))
                                	{
                                		//manGoal.manMovesInState = state.manMovesInState.substring(state.manMovesInState.length() - 1);
                                		manGoal.manMovesInState += 'a';
                                	...
                                	break;
                                case 2:
                                	if(!state.manMovesInState.endsWith(Character.toString(leftDirection).toUpperCase()))
                                	{
                                		//manGoal.manMovesInState = state.manMovesInState.substring(state.manMovesInState.length() - 1);
                                		manGoal.manMovesInState += 'a';
                                	...
                                	break;
                                case 3:
                                	if(!state.manMovesInState.endsWith(Character.toString(rightDirection).toUpperCase()))
                                	{
                                		//manGoal.manMovesInState = state.manMovesInState.substring(state.manMovesInState.length() - 1);
                                		manGoal.manMovesInState += 'a';
                        ...
                        else //See if the man can get to the correct position
                        {
                            ...
                            if (manFinder.search(state, manGoal))
                            {
                                ...
                                manDone.manMovesInState = "a" + manFinder.getMoves();
